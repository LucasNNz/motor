...
app = FastAPI(title="Corvo Image Engine", version="0.12.6")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def get_system_info():
    info = {
        ...
        "notes": [
            "V0.12.6 é guia-first e browser-first: o refinador principal é executado no navegador via WebGPU/WASM. Backends nativos locais são apenas legado opcional."
        ],
    }
    try:
        import torch
...
@app.get("/api/health")
def health():
    return {"ok": True, "jobs": len(jobs), "version": "0.12.6", **runtime_status()}
