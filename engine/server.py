...
app = FastAPI(title="Corvo Image Engine", version="0.12.12")

@app.middleware("http")
async def corvo_build_headers(request, call_next):
    response = await call_next(request)
    response.headers["Cache-Control"] = "no-store, max-age=0"
    response.headers["X-Corvo-Build"] = "0.12.12-root-clean"
    return response
...
def get_system_info():
    info = {
        ...
        "notes": [
            "V0.12.12 é guia-first e browser-first: o refinador principal é executado no navegador via WebGPU/WASM. Backends nativos locais são apenas legado opcional."
        ],
    }
    try:
        import torch
...
@app.get("/api/health")
def health():
    return {"ok": True, "jobs": len(jobs), "version": "0.12.12", **runtime_status()}
